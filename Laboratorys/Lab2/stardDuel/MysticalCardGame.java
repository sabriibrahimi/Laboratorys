...
public class MysticalCardGame {
    ...
    public static void startDuel(SLL<Card> firstSorcererCards, SLL<Card> secondSorcererCards) {
        ...
        SLLNode<Card> bestCard = curr;

        while(curr != null) {
            if(curr.element.significance() > bestCard.element.significance()){
                bestCard = curr;
            }
            curr = curr.succ;
        }
        ...
        SLLNode<Card> current = secondSorcererCards.getFirst();

        for(int i=0;i<middI;i++){
            current = current.succ;
        }

        firstSorcererCards.delete(bestCard);

        secondSorcererCards.insertBefore(bestCard.element, current);
